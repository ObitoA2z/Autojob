...
class CadremploiScraper(BaseScraper):
    ...
    async def search(self, keywords: str, location: str, max_results: int = 25) -> list[ScrapedJob]:
        ...
        async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=30) as client:
            ...
            while len(jobs) < max_results:
                ...
                url = f"{self.BASE_URL}/emploi/liste_offres?" + urlencode(params)
                try:
                ...
                if found_in_page == 0:
                    cards = soup.select(
                        "article.offer-card, div.offer-item, li[data-id], "
                        "[class*='offer-card'], [class*='jobcard'], [class*='job-card']"
                    )
                    for card in cards:
                        try:
                            title_el = card.select_one("h2 a, h3 a, a[class*='title'], [class*='title'] a")
                            if not title_el:
                                continue
    ...
    async def apply(self, job_url: str, cv_path: str, cover_letter: str = "") -> dict:
        return {"success": False, "message": "Cadremploi: candidature manuelle sur cadremploi.fr"}
