...
HEADERS = {
    ...
    "Referer": "https://www.jobijoba.com/fr/",
}


class JobijobasScraper(BaseScraper):
    """Jobijoba - agrégateur d'offres d'emploi français."""
    platform_name = "jobijoba"
    ...
    async def search(self, keywords: str, location: str, max_results: int = 25) -> list[ScrapedJob]:
        ...
        async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=30) as client:
            ...
            while len(jobs) < max_results:
                ...
                url = f"{self.BASE_URL}/fr/find_jobs.php?" + urlencode(params)
                try:
                ...
                if found_in_page == 0:
                    cards = soup.select(
                        "li.job, div.job-item, article.job, "
                        "[class*='job-result'], [class*='joboffer'], [data-job-id]"
                    )
                    for card in cards:
                        try:
                            ...
                            company_el = card.select_one("[class*='company'], [class*='employer'], [class*='entreprise']")
                            company = company_el.get_text(strip=True) if company_el else ""
                            loc_el = card.select_one("[class*='location'], [class*='lieu'], [class*='city']")
    ...
    async def apply(self, job_url: str, cv_path: str, cover_letter: str = "") -> dict:
        return {"success": False, "message": "Jobijoba: candidature manuelle sur jobijoba.com"}
